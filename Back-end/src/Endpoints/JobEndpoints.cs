...
public static class JobEndpoints
{
    ...
    {
        routes.MapGet("/jobs", (HttpContext ctx, JobService jobService) =>
        {
            var filters = ctx.Request.Query.ToDictionary(query => query.Key, query => query.Value.ToString());
        ...
        routes.MapGet("/saved-jobs", (HttpContext ctx, JobService jobService) =>
        {
            var filters = ctx.Request.Query.ToDictionary(query => query.Key, query => query.Value.ToString());
